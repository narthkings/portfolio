                ...
                {({ values: details, handleSubmit, handleChange, handleBlur, errors, touched }) => (
                    <Form onSubmit={handleSubmit} >
                        <Box textAlign={'center'} display={'flex'} flexDirection={'column'} justifyContent={'center'} marginTop={'5rem'}>
                            <FormControl id='name'>
                                <Input id='name' name="name" onChange={handleChange} onBlur={handleBlur} value={details.name} variant='filled' placeholder='Enter Name' size='lg' width={'50%'} />
                                <FormHelperText color="red">{errors.name && touched.name && errors.name}</FormHelperText>
                            </FormControl>

                            <FormControl id='email'>
                                <Input id="email" name='email' onChange={handleChange} value={details.email} onBlur={handleBlur} variant='filled' placeholder='Enter Email address' size='lg' width={'50%'} marginTop={'2rem'} />
                                <FormHelperText color="red">{errors.email && touched.email && errors.email}</FormHelperText>
                            </FormControl>

                            <FormControl id="message">
                                <Textarea id="message" name='message' onChange={handleChange} onBlur={handleBlur} value={details.message} variant='filled' placeholder="Amazing Portfolio, I’d like you to work with my team on a project ………" width={'50%'} size="lg" height={'8rem'} marginTop={'2rem'} />
